...
y_MNIST = [mean(sc_nc_ORL);...
     mean(sc_nsc_ORL(:,1));...
     mean(sc_nsc_ORL(:,2));...
     mean(sc_nsc_ORL(:,3));...
     mean(sc_nn_ORL);...
     mean(sc_pbp_ORL);...
     mean(sc_pmse_ORL)]*100;
c = {'NC','NSC2','NSC3','NSC5','NN','P-BP','P-MSE'};
bar(y_MNIST)
barvalues;
ylim([0 120]);
...
t_nsc_ORL_mat = cell2mat(t_nsc_ORL);
y_MNIST = [mean(cell2mat(t_nc_ORL));...
     mean(t_nsc_ORL_mat(:,1));...
     mean(t_nsc_ORL_mat(:,2));...
     mean(t_nsc_ORL_mat(:,3));...
     mean(cell2mat(t_nn_ORL));...
     mean(cell2mat(t_pbp_ORL));...
     mean(cell2mat(t_pmse_ORL))]*1000;
c = {'NC','NSC2','NSC3','NSC5','NN','P-BP','P-MSE'};
bar(y_MNIST)
barvalues;
set(gca,'xticklabel',c)
...
h_pca_orl = plot(target_dimension,mean(sc_nc_ORL_pca)*100,...
                 target_dimension,mean(sc_nsc_ORL_pca(:,:,1))*100,...
                 target_dimension,mean(sc_nsc_ORL_pca(:,:,2))*100,...
                 target_dimension,mean(sc_nsc_ORL_pca(:,:,3))*100,...
                 target_dimension,mean(sc_nn_ORL_pca)*100,...
                 target_dimension,mean(sc_pbp_ORL_pca)*100,...
                 target_dimension,mean(sc_pmse_ORL_pca)*100);
...
x_ORL = [mean(sc_nc_ORL),mean(sc_nsc_ORL), mean(sc_nn_ORL), mean(sc_pbp_ORL),mean(sc_pmse_ORL)]'*100;
y_ORL = [mean(cell2mat(t_nc_ORL)),...
         mean(cell2mat(t_nsc_ORL)),...
         mean(cell2mat(t_nn_ORL)),...
         mean(cell2mat(t_pbp_ORL)),...
         mean(cell2mat(t_pmse_ORL))]';
...
x_ORL_pca = [mean(sc_nc_ORL_pca(:,20)),...
            mean(sc_nsc_ORL_pca(:,20,1)),...
            mean(sc_nsc_ORL_pca(:,20,2)),...
            mean(sc_nsc_ORL_pca(:,20,3)),...
            mean(sc_nn_ORL_pca(:,20)), mean(sc_pbp_ORL_pca(:,20)),mean(sc_pmse_ORL_pca(:,20))]'*100;
t_nsc_ORL_pca_mat = cell2mat(t_nsc_ORL_pca);
y_ORL_pca = [mean(cell2mat(t_nc_ORL_pca(:,20))),...
             mean(t_nsc_ORL_pca_mat(:,1,20)),...
             mean(t_nsc_ORL_pca_mat(:,2,20)),...
             mean(t_nsc_ORL_pca_mat(:,3,20)),...
             mean(cell2mat(t_nn_ORL_pca(:,20))),...
             mean(cell2mat(t_pbp_ORL_pca(:,20))),...
             mean(cell2mat(t_pmse_ORL_pca(:,20)))]';
